...
function refreshCurrentDimensions(){
...
window.addEventListener("resize", (event) => {
    event.preventDefault();
    refreshCurrentDimensions();
    updateCanvasBackstoreDimensions();
  })

function updateCanvasBackstoreDimensions(){
    design.setDimensions({
        width: 5000,
        height: 5000*(currentHeight/currentWidth)
      },{
        backstoreOnly: true
      });
}

var design = new fabric.Canvas('kutas', { containerClass: 'kutas'});
updateCanvasBackstoreDimensions();
design.setDimensions({
    width: "100%",
    height: "100%"
...
design.on('mouse:down:before', function(opt) {
    ...
    design.on('mouse:move:before', function(opt) {
    if (this.isDragging) {
      var e = opt.e;
    ...
    design.on('mouse:up:before', function(opt) {
        var evt = opt.e;
        if (evt.button == 2) {
